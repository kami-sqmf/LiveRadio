...
#ifdef WIN32
#include <windows.h>
/* Airplay Radio: key files are supplied as UTF-8 by the managed mod. */
static FILE *keyfile_open(const char *path, const char *mode) {
    int n = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, path, -1, NULL, 0);
    if (!n) return NULL;
    wchar_t *wide = calloc(n, sizeof(wchar_t));
    if (!wide) return NULL;
    MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, path, -1, wide, n);
    FILE *file = _wfopen(wide, mode[0] == 'r' ? L"r" : L"w");
    free(wide);
    return file;
}
#else
#define keyfile_open fopen
#endif
