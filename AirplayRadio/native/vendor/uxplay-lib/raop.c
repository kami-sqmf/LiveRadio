 ...
 /* this rejects messages from _airplay._tcp for video streaming protocol unless bool raop->hls_support is true*/   
    ...
    *response = http_response_create();
    ...
    if (!conn->have_active_remote) {
        ...
        const char *remote_id = http_request_get_header(request, "DACP-ID");
        if (active_remote && remote_id) {
            conn->have_active_remote = true;
            if (raop->callbacks.remote_control) {
                char peer[80] = {0};
                utils_ipaddress_to_string(conn->remotelen, conn->remote, conn->zone_id, peer, sizeof(peer));
                raop->callbacks.remote_control(raop->callbacks.cls, conn, peer, remote_id, active_remote);
            }
...
conn_destroy(void *ptr) {
    ...
    if (raop->callbacks.remote_control_closed)
        raop->callbacks.remote_control_closed(raop->callbacks.cls, conn);
...
raop_destroy(raop_t *raop) {
    if (raop) {
        ...
        /* Airplay Radio: partial startup can fail before httpd is allocated. */
        if (raop->httpd) raop_stop_httpd(raop);
