                           ...
                           char **response_data, int *response_datalen)
{
    ...
    if (!strcmp(content_type, "text/parameters")) {
        ...
        free(datastr);
    } else if (!strcmp(content_type, "image/none")) {
        if (conn->raop_rtp)
            raop_rtp_set_coverart(conn->raop_rtp, NULL, 0);
        else if (raop->callbacks.audio_stop_coverart_rendering)
        ...
        if (datalen > 2 * 1024 * 1024) {
            http_response_init(response, "RTSP/1.0", 413, "Artwork too large");
            return;
        }
