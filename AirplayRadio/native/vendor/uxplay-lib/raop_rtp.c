...
raop_rtp_resend_callback(void *opaque, unsigned short seqnum, unsigned short count)
{
    ...
    if (raop_rtp->callbacks.audio_resend_requested)
        raop_rtp->callbacks.audio_resend_requested(raop_rtp->callbacks.cls, count);
...
raop_rtp_set_coverart(raop_rtp_t *raop_rtp, const char *data, int datalen)
{
    ...
    if (datalen < 0 || datalen > 2 * 1024 * 1024 || (datalen > 0 && !data)) return;
    /* A non-null, zero-length event clears artwork in order on the RTP worker. */
    unsigned char *coverart = (unsigned char *) malloc(datalen > 0 ? datalen : 1);
    if (!coverart) return;
    if (datalen > 0) memcpy(coverart, data, datalen);
